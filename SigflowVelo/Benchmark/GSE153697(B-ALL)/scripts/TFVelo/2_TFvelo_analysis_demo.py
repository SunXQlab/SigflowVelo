...
def get_pseudotime(adata):
    ...
    TFv.pl.scatter(adata, basis=args.basis, color='velocity_pseudotime', cmap='gnuplot', fontsize=20, save=False)
    return adata
...
def show_adata(args, adata, save_name, show_all=0):
    ...
    if len(adata.obs['condition'].cat.categories) > 10:
    ...
    TFv.pl.velocity_embedding_stream(adata, vkey='velocity', use_derivative=False, density=2, basis=args.basis, \
        ...
        legend_loc=legend_loc, save=False)
...
def main(args):
    ...
    if (args.basis=='umap') and ('X_umap' not in adata.obsm.keys()):
        ...
        if args.dataset_name == 'hesc1':
            ...
            sc.tl.umap(adata)  
            sc.pl.umap(adata, color='condition', save=False)
    ...
    TFv.tl.velocity_graph(adata_copy, basis=None, vkey='velocity', xkey='M_total', n_jobs=1)
    ...
    print(adata_copy)
    adata_copy.write(args.data_path + 'adata_with_TFVelo.h5ad')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        parser.add_argument('--dataset_name', type=str, default="pancreas",
        ...
        parser.add_argument('--spearmanr_thres', type=float, default=0.5, help='min spearmanr')
        parser.add_argument('--save_name', type=str, default='_demo', help='save_name')
        args = parser.parse_args()
    else:
        class Args:
            dataset_name = "Maglignant_cells"
            layer = "M_total"
            basis = "umap"
    ...
    args.data_path = "../../results/TFVelo/adata/"
    print('------------------------------------------------------------')
    print(args)
    main(args)
